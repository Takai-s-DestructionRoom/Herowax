...
public:
	...
	//life:秒数指定なので注意
	void AddRing(Vector3 position,uint32_t addNum, float life, Color color, float startRadius, float endRadius, float minScale, float maxScale,
		float minVeloY, float maxVeloY, Vector3 minRot = {}, Vector3 maxRot = {}, float growingTimer = 0.f);

	//パーティクルの追加
	void AddSimple(Vector3 position,uint32_t addNum, float life, Color color, float minScale, float maxScale,
