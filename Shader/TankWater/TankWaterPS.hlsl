...
ConstantBuffer<TankData> bTank : register(b10);
...
float4 main(VSOutputBasic input) : SV_TARGET
{
    ...
    float wav = sin(((input.wpos - bTank.centerPos).x + bTank.timer) * bTank.frequency);
    if (input.wpos.y >= bTank.centerPos.y + bTank.upper + bTank.amplitude * wav)
    {
        discard;
    }
